      ...
      <div className="hero-auction-content bg-orange-200 rounded-lg sm:rounded-xl shadow-xl overflow-hidden border-2 border-red-400">
        ...
        <div className="flex flex-col lg:grid lg:grid-cols-5">
          ...
          <div className="lg:col-span-3 relative flex items-center justify-center overflow-hidden aspect-[16/9] lg:aspect-auto lg:min-h-[400px]">
            ...
            {/* LIVE/ENDED Badge - Mobile Responsive */}
            <div className="absolute top-1 left-1 sm:top-2 sm:left-2">
              {isAuctionEnded ? (
                <span className="bg-gray-600 text-white px-2 py-1 sm:px-4 sm:py-2 rounded-full text-xs sm:text-sm font-bold shadow-lg flex items-center gap-1 sm:gap-2">
                  🏁 AUCTION ENDED
                </span>
              ) : (
          ...
          <div className="lg:col-span-2 flex flex-col justify-center p-3 sm:p-6">
            ...
            <div className="space-y-1 sm:space-y-3 mb-2 sm:mb-4">
              ...
              <div className={`${isAuctionEnded ? 'bg-gray-100 border-gray-300' : 'bg-amber-100 border-amber-300'} border-2 rounded-lg p-2 sm:p-4 overflow-hidden`}>
                <p className={`${isAuctionEnded ? 'text-gray-700' : 'text-amber-900'} text-xs font-bold uppercase tracking-wider mb-0.5 sm:mb-1`}>
                  {isAuctionEnded ? '🏁 Auction Ended' : '🎉 Pledge Your Bid From'}
                </p>
                <p className={`text-xl sm:text-3xl lg:text-4xl font-black ${isAuctionEnded ? 'text-gray-700' : 'text-amber-900'} mb-0.5 sm:mb-1`}>
            ...
            <Link
              ...
              className={`cta-button relative ${isAuctionEnded ? 'bg-gray-600' : 'bg-red-600'} text-white px-3 sm:px-8 py-2.5 sm:py-4 rounded-lg sm:rounded-xl text-sm sm:text-lg font-black uppercase tracking-wide shadow-lg flex items-center justify-center gap-1 sm:gap-3 overflow-hidden`}
            >
              {!isAuctionEnded && <span className="cta-shimmer"></span>}
              <span className="relative z-10 text-center">
                {isAuctionEnded ? 'VIEW AUCTION RESULTS' : 'JOIN THE BID NOW!'}
              </span>
            </Link>
